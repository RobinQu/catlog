...
Container.get = function(category, options) {
  ...
  if(!options.console) {
    c.addLogger(require("./loggers/console"), options);
  }
  return c;
};

Container.levels = ["trace", "debug", "info", "log", "warn", "error"];

// Instance members
Container.prototype.record = function() {
  var level, messages, stack;
  
  level = arguments[0] || "info";
  stack = stackTrace.get(this.record).slice(1);
  messages = this.preprocess(level, slice.call(arguments, 1) || [], stack);
  if(this.printf) messages = [printf.apply(null, messages)];
